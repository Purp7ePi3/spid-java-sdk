 ...
 * 1. Firma digitale valida (certificato IdP) ← OBBLIGATORIA
 * 2. InResponseTo corrisponde alla request originale
 * 3. Destination corrisponde all'ACS URL
 * 4. StatusCode = Success
 * 5. Audience corrisponde all'entityId del SP
 * 6. NotBefore / NotOnOrAfter nell'intervallo valido
 */
public class SamlResponseValidator {
  ...
  public ValidationResult validate(String samlResponseXml,
    ...
    // 1. Verifica firma digitale — OBBLIGATORIA
    // Se il certificato IdP non è fornito o non è valido, la validazione fallisce
    if (idpCertBase64 == null || idpCertBase64.isBlank()) {
      errors.add("Certificato IdP mancante — impossibile verificare la firma");
      return ValidationResult.failed(errors);
    }
    ...
    try {
      ...
      log.debug("Firma XML IdP valida");
    } catch (SecurityException e) {
      errors.add("Firma digitale non valida: " + e.getMessage());
      // Firma non valida → blocco immediato, non continuare
      return ValidationResult.failed(errors);
    } catch (Exception e) {
      errors.add("Errore nella verifica della firma: " + e.getMessage());
      return ValidationResult.failed(errors);
    }
